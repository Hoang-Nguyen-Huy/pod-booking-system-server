...
(
    ...
    `status`           ENUM('Pending', 'Rejected', 'Successfully') NOT NULL DEFAULT 'Pending',
    `createdAt`        DATETIME    NOT NULL,
    `updatedAt`        DATETIME    NOT NULL,
...
-- =============================================
    ...
    VALUES (UUID(),
           ...
           (SELECT id FROM account WHERE email='b@flexipod.com' LIMIT 1), 60000, 0, NOW(), DATE_ADD(NOW(), INTERVAL 1 HOUR), 'Successfully', NOW(), NOW() ),
(UUID(),
